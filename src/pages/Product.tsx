    ...
    <section className="pt-32 pb-20 relative">
      <div className="container mx-auto px-6 text-center">
        ...
        <p className="text-lg md:text-xl text-muted-foreground mb-10 max-w-2xl mx-auto animate-fade-in-up delay-100">
          From idea to live app — with preview, integrations, and deploy-ready code.
        </p>
        <Button variant="gradient" size="xl" className="animate-fade-in-up delay-200" asChild>
    ...
    <section className="py-24 relative">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 max-w-4xl mx-auto">
          {buildTypes.map((type) => (
            <div key={type.title} className="content-card p-5 glow-hover text-center">
              <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center mx-auto mb-3">
                <type.icon className="w-5 h-5 text-primary" />
              </div>
              <span className="text-foreground text-sm font-medium">{type.title}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
          {differentiators.map((item) => (
            <div key={item.text} className="content-card px-5 py-3 glow-hover flex items-center gap-3">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
          {audiences.map((audience) => (
            <div key={audience.title} className="content-card px-6 py-4 glow-hover flex items-center gap-3">
              <audience.icon className="w-5 h-5 text-primary" />
              <span className="text-foreground font-medium">{audience.title}</span>
            </div>
          ))}
        </div>
      </div>
